...
class AccountController extends Controller
{
    ...
    public function addcategoryAction(Request $request) {
        ...
        if ($form->isValid()) {
            ...
            // redirect to categories config page
    ...
    public function addsubcategoryAction(Request $request, $id) {
        ...
        if ($form->isValid()) {
            ...
            $category->setParent($em->getRepository('OrthIndexBundle:Customcategory')->findOneBy(array('id' => $id)));
    ...
    public function myusermanagerAction($id, Request $request) {
        ...
        if ( $request->request->all()) {
            ...
            foreach ($formPost as $key => $value) {
                ...
                if ( $userPerm == NULL ) {
                    ...
                    $articleCats1 = $em->getRepository('OrthIndexBundle:Customcategory')->findBy(array('parentId' => $articleCat->getId()));
                    ...
                    foreach ($articleCats1 as $articleCat) {
                        ...
                        $articleCats2 = $em->getRepository('OrthIndexBundle:Customcategory')->findBy(array('parentId' => $articleCat->getId()));
        ...
        $categories = $em->getRepository('OrthIndexBundle:Customcategory')->findBy(array('customerRef' => $user->getCustomerRef(), 'parentId' => NULL));
    ...
    public function deletecategoryAction($id, Request $request) {
        ...
        $subcategory = $em->getRepository('OrthIndexBundle:Customcategory')->findBy(array('customerRef' => $user->getCustomerRef(), 'parentId' => $id));

        if($subcategory != NULL) {
            foreach ($subcategory as $sub1) {
                $arrayToRefresh[] = $sub1->getId();

                $subcategory2 = $em->getRepository('OrthIndexBundle:Customcategory')->findBy(array('customerRef' => $user->getCustomerRef(), 'parentId' => $sub1->getId()));
    ...
    public function approvalsAction() {
      ...
      $approvals = $em->getRepository('OrthIndexBundle:Approvals')->findBy(array('requestedBy' => $user->getId()));

      return $this->render('OrthIndexBundle:Account:approvals.html.twig', array('approvals' => $approvals));
    ...
    public function approvalAction($id) {
      ...
      $approval = $em->getRepository('OrthIndexBundle:Approvals')->findOneById($id);
      $items = $em->getRepository('OrthIndexBundle:ShoppingCart')->findBy(array('approvalRef' => $id));
      $cartItems = $em->getRepository('OrthIndexBundle:ShoppingCart')->buildCart($items, $user);

      return $this->render('OrthIndexBundle:Account:approval.html.twig', array('approval' => $approval, 'items' => $cartItems));
